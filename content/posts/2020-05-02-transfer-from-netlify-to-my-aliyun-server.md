...
因为之前部署在 Netlify 上，将我期望的域名 charlesfeng.cn 的 DNS 解析修改为 Netlify 的，然后使用 Netlify DNS CNAME 映射到 Netlify 自己的服务器上。「境内域名 + 境外 DNS + 境外服务器」不需要备案，可以正常通过域名访问。而我迁移到自己的服务器后，因为使用了境内服务器，所以如果想通过域名访问，就需要备案。而我不太想加上备案号...所以进行了一些失败的尝试...🥱
...
在经过上述三种尝试后，我人已经彻底晕菜了，但是不甘心为啥需要备案，于是打开[阿里云备案](https://help.aliyun.com/document_detail/147840.html?spm=5176.13841030.J_8179121650.8.379a5c696RBVSG)，想了解下备案的原因到底是什么。

>您是否需要备案主要看您的网站等互联网信息服务解析到的服务器是否在中国内地（大陆），如果服务器在中国香港、中国澳门、中国台湾及其他国家和地区，则不需要备案。

根据说明，备案与否只与「服务器」有关。所以啊！朋友们！要先看清楚再尝试啊！我就是吃了想当然的亏啊！不光浪费精力，还浪费钱啊！
...
最终还是因为境外 Netlify 速度不🉑️，选择使用了全境内的方式。（其实之前使用谷歌云的服务器时，境内访问速度还行，但是终端远程连接时简直卡爆😭...所以实在不想用境外服务器）
