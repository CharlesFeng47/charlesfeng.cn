...
template: post
thumbnail: '../thumbnails/sonarqube.svg'
...
>
> 但是通过试验，`chown -R sonarqube`  只修改软链接，但是 `chown -R sonarqube/` 则会修改实际目录内的权限，但是对实际目录 `chown -R sonarqube-8.0` 和 `chown -R sonarqube-8.0/` 效果相同。应该是因为 `chown -R sonarqube` 递归处理的是软链接文件 inode 号，而`chown -R sonarqube/` 递归处理的软链接文件指向的实际文件的 inode 号吧。
