...
前端团队可以通过模拟所需的数据结构轻松地测试其应用程序。服务器准备就绪后，可以翻转该开关，以使客户端应用程序从实际的 API 加载数据。
